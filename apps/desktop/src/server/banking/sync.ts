...
async function syncAccountTransactions(
  ...
  // FOREVER. Re-fetched rows are idempotent via the (source, external_id)
  // unique index, so the overlap only ever inserts genuine late arrivals.
  ...
  const accountWatermark = latestExisting
    ? new Date(
        new Date(latestExisting.occurredAt).getTime() +
