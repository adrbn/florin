...
/**
 * Read-only JSON feed for the native iOS client.
 *
 * It returns the exact view models the v2 React screens already consume
 * (`V2Account`, `V2Tx`, and the query-layer result types) rather than a new
 * shape invented for the API. That is deliberate: those types were designed to
 * be serializable so one set of numbers can drive the web UI and a native one
 * without a second implementation of the arithmetic drifting away from the
 * first. Every figure here comes from the same `queries.*` calls the dashboard
 * uses, so the phone can never disagree with the browser.
 */
...
export async function GET(request: Request) {
  ...
  // from the amortization schedule. Ship that figure so a client summing the
  // account rows lands on the same net worth as the headline.
