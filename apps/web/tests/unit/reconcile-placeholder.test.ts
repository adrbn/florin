...
describe('isPlaceholderOf', () => {
  it('matches the bank re-booking an instant transfer with the counterparty', () => {
    ...
    expect(
      isPlaceholderOf('VIREMENT INSTANTANE CREDIT', 'VIREMENT INSTANTANE DE MME DUPONT MARIE'),
    ).toBe(true)
    expect(
  ...
  it('accepts a bare rail prefix with nothing after it', () => {
    expect(isPlaceholderOf('VIREMENT INSTANTANE', 'VIREMENT INSTANTANE DE MME DUPONT')).toBe(true)
  })
  ...
  it('refuses when the earlier label is not the shorter one', () => {
    expect(
      isPlaceholderOf('VIREMENT INSTANTANE DE MME DUPONT MARIE', 'VIREMENT INSTANTANE CREDIT'),
    ).toBe(false)
