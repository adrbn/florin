...
describe('getLeftToSpendThisMonth — salary category detection', () => {
  it('a one-off inflow ≥ 500 does not hijack the salary category from the real salary', async () => {
    ...
    // NOT to the 560.00 of miscellaneous inflows.
    // The ceiling is every euro that came in, not the paycheck alone: the
    // salary plus the 560.00 one-off, which is real money either way.
    expect(lts.monthIncome).toBeCloseTo(2500.00 + 560.00, 2)
    expect(lts.leftToSpend).toBeCloseTo(2500.00 + 560.00 - 400, 2)
  })
