 ...
 */
export function CategoriesEditor({ groups }: CategoriesEditorProps) {
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-2">
        ...
        {adding ? (
          <NewCategoryForm groupId={group.id} onDone={() => setAdding(false)} />
        ) : (
          <Button
            size="xs"
            variant="ghost"
...
function NewCategoryForm({ groupId, onDone }: { groupId: string; onDone: () => void }) {
  const [pending, startTransition] = useTransition()
...
function NewGroupForm({ onDone }: { onDone: () => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setError(null)
    startTransition(async () => {
      const result = await createCategoryGroup({ name: name.trim(), kind })
      if (!result.success) {
        setError(result.error ?? 'Failed')
        return
      }
...
function EditGroupForm({ group, onDone }: { group: GroupRow; onDone: () => void }) {
  const [pending, startTransition] = useTransition()
