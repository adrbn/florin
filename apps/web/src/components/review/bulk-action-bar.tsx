...
import { type CategoryOption, CategoryPicker } from '@/components/ui/category-picker'
import {
 ...
 */
export function BulkActionBar({ selectedIds, categoryOptions, onDone }: BulkActionBarProps) {
  const [pending, startTransition] = useTransition()
  const count = selectedIds.length

  const onCategorize = (categoryId: string | null) => {
    startTransition(async () => {
      await bulkUpdateTransactionCategory(selectedIds, categoryId)
      onDone()
    })
  }
  ...
  const onApprove = () => {
    startTransition(async () => {
      await bulkApproveTransactions(selectedIds)
      onDone()
    })
  }
