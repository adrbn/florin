...
import type { CategoryOption } from '@/components/ui/category-picker'
 ...
 */
export function TransactionsTable({ rows, categoryOptions, emptyMessage }: TransactionsTableProps) {
  ...
  return (
    <div style={style}>
      <div className="divide-y divide-border/60">
        ...
        {rows.map((row) => (
          <TransactionRow key={row.id} row={row} categoryOptions={categoryOptions} />
        ))}
      </div>
    </div>
  )
...
function TransactionRow({ row, categoryOptions }: TransactionRowProps) {
    ...
    <div className="flex flex-col gap-1.5 px-3 py-2.5 text-xs hover:bg-muted/40 md:grid md:grid-cols-[var(--col-date)_minmax(0,1fr)_var(--col-account)_var(--col-category)_var(--col-amount)_var(--col-actions)] md:items-center md:gap-3 md:py-2">
      ...
      <div className="flex min-w-0 items-center justify-between gap-2 md:contents">
        ...
        <div className="flex shrink-0 items-center md:justify-self-center">
          <DeleteTransactionButton transactionId={row.id} payee={row.payee} />
        </div>
      </div>
    </div>
  )
