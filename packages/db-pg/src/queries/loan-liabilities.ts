 ...
 *
 * We need the "payments made" count per loan to walk the amortization
 * schedule to the right row. That count = number of mirror transactions on
 * the loan account (rows with transfer_pair_id IS NOT NULL). Each real
 * payment creates exactly one such row via the loan-mirror mechanism, so
 * this is authoritative — manual balance adjustments (no pair id) are
 * correctly excluded.
 *
 * The function is idempotent and pure-ish: same input accounts + same DB
 * state → same output. Non-loan accounts in the input are silently ignored
