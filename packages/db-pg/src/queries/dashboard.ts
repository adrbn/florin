 ...
 * to spend". Returns zeros + null ids when the user hasn't been paid recently
...
export async function getLeftToSpendThisMonth(db: PgDB): Promise<LeftToSpend> {
  ...
  const monthKey = sql`to_char(${transactions.occurredAt}, 'YYYY-MM')`
...
export async function getSavingsRates(db: PgDB): Promise<SavingsRates> {
  ...
  for (const w of windows) {
    ...
    const start = startOfMonth(addMonths(new Date(), -w.months))
    const end = endOfMonth(addMonths(new Date(), -1))
    const rows = await db
      .select({
