...
// SQLite twin of db-pg/recurring.ts. Dates are ISO 'YYYY-MM-DD' strings,
// amounts are real numbers.
  ...
  try {
    const [row] = await db
      ...
      .values({
        ...
        endDate: d.endDate ? isoDate(d.endDate) : null,
      })
...
export async function updateRecurringRuleMutation(
  ...
  try {
    ...
    if (input.amount !== undefined) set.amount = Number(input.amount)
    ...
    if (input.startDate !== undefined && input.startDate) {
      set.startDate = isoDate(new Date(input.startDate))
    }
    if (input.endDate !== undefined) {
      set.endDate = input.endDate ? isoDate(new Date(input.endDate)) : null
    }
      ...
      const future = await db
        ...
        .where(
          and(
            ...
            sql`${transactions.occurredAt} > ${todayIso}`,
          ),
        )
      if (future.length > 0) {
...
async function insertOccurrence(
  ...
  const recurrenceKey = `${rule.id}:${isoDate(d)}`
  const occurredAt = isoDate(d)
...
export async function materializeScheduledTransactions(
  ...
  try {
    ...
    const horizonMs = Date.UTC(
      now.getUTCFullYear(),
      now.getUTCMonth() + HORIZON_MONTHS,
      now.getUTCDate(),
    )
    ...
    for (const rule of rules) {
      const start = new Date(`${String(rule.startDate).slice(0, 10)}T00:00:00Z`)
      if (Number.isNaN(start.getTime())) continue
      const end = rule.endDate ? new Date(`${String(rule.endDate).slice(0, 10)}T00:00:00Z`) : null
      const lastMat = rule.lastMaterializedDate
        ? new Date(`${String(rule.lastMaterializedDate).slice(0, 10)}T00:00:00Z`)
        : null

      const lowerMs = Math.max(
        toUtcMidnight(start),
        todayMs,
        lastMat && !Number.isNaN(lastMat.getTime()) ? toUtcMidnight(lastMat) + 86_400_000 : 0,
      )
      const endMs = end && !Number.isNaN(end.getTime()) ? toUtcMidnight(end) : null
      ...
      if (occ.length > 0) {
        await db
          ...
          .set({ lastMaterializedDate: isoDate(occ[occ.length - 1]), updatedAt: new Date().toISOString() })
