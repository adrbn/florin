...
import type { ActionResult, LoanSettingsInput } from '../../types/index'
...
interface LoanDetailsCardProps {
   ...
   */
  paymentsMade: number
  onUpdateLoanSettings: (input: LoanSettingsInput) => Promise<ActionResult>
  onSetCategoryLoanLink: (input: { categoryId: string; loanAccountId: string | null }) => Promise<ActionResult<{ touched: number }>>
}
...
export function LoanDetailsCard({
  ...
  const onLinkCategory = (catId: string) => {
    ...
    startLinkTransition(async () => {
      const result = await onSetCategoryLoanLink({
        categoryId: catId,
  ...
  const onSave = () => {
    ...
    startTransition(async () => {
      const result = await onUpdateLoanSettings({
        id: account.id,
